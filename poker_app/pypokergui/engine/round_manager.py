...
from poker_app.pypokergui.engine.game_evaluator import GameEvaluator
...
class RoundManager:
  ...
  def __blind_transaction(cls, player, small_blind, sb_amount):
    action = Const.Action.SMALL_BLIND if small_blind else Const.Action.BIG_BLIND   #moje else Const.Action.BIG_BLIND
  ...
  def __showdown(cls, state):
    ...
    result_message = MessageBuilder.build_round_result_message(state["round_count"], winners, state)
    state["table"].reset()
    state["street"] += 1
