...
class Player:
  ...
  ACTION_CHECK_STR = "CHECK"
  ACTION_ALLIN_STR = "all_in"
  ...
  def paid_sum(self):
    pay_history = [h for h in self.action_histories if h["action"] not in ["FOLD", "ANTE", "CHECK"]]
  ...
  def deserialize(cls, serial):
    ...
    player = cls(serial[1], serial[2], serial[0])
  ...
  def __allin_history(self, bet_amount, add_amount):
    return {
        "action" : self.ACTION_ALLIN_STR,
        "amount" : bet_amount,
        "paid" : bet_amount - self.paid_sum(),
        "add_amount" : add_amount
    }
  ...
  def __blind_history(self, small_blind, sb_amount):
      ...
      amount = sb_amount if small_blind else sb_amount
      add_amount = sb_amount  # Dla obu blindów add_amount to sb_amount
      return {
          "action": action,
          "amount": amount,
          "add_amount": add_amount
