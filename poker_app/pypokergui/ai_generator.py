...
"""Assert passed script satisfies requirements for PyPokerGUI
    ...
    returns child instance of poker_app.pypokergui.players.BasePokerPlayer.
"""
def healthcheck(script_path, quiet=False):
    ...
    try:
        ...
        if not quiet: print('"setup_ai" method was not found in [ %s ].(Exception=%s)' % (script_path, str(e)))
        status = False
    ...
    try:
        ...
        if not quiet: print('Exception [ %s ] was raised when your "setup_ai" method invoked' % str(e))
        status = False
