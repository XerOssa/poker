...
from poker_app.pypokergui.utils.game_state_utils import deepcopy_game_state
...
class Emulator(object):
    ...
    def generate_initial_game_state(self, players_info):
        ...
        table.dealer_btn = len(table.seats.players)-2
        return {
            "round_count": 0,
...
class Event:
    ...
    @classmethod
    def create_new_street_event(cls, message):
        return {
                "type": cls.NEW_STREET,
    ...
    @classmethod
    def create_ask_player_event(cls, message):
    ...
    @classmethod
    def create_round_finish_event(cls, message):
        player_info = lambda info: { "uuid": info["uuid"], "stack": info["stack"] }
        return {
                "type": cls.ROUND_FINISH,
                "round_state": message["round_state"],
                "winners": [player_info(info) for info in message["winners"]]
                }

    @classmethod
    def create_game_finish_event(cls, message):
        player_info = lambda info: { "uuid": info["uuid"], "stack": info["stack"] }
        return {
                "type": cls.GAME_FINISH,
